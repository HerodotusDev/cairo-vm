use cairo_lang_sierra::program::{Program as SierraProgram, Statement, StatementIdx};
...
pub fn compute_bytecode_segment_lengths(
    ...
    let functions_segment_start_statements = find_functions_segments(sierra_program)?;
    let mut segment_start_offsets =
        functions_statement_ids_to_offsets(cairo_program, &functions_segment_start_statements);
    segment_start_offsets.extend(consts_segments_offsets(cairo_program, bytecode.len()));

    Ok(NestedIntList::Node(
        get_segment_lengths(&segment_start_offsets, bytecode.len())
            .iter()
...
fn find_functions_segments(
    sierra_program: &SierraProgram,
) -> Result<Vec<usize>, SegmentationError> {
    // Get the set of function entry points.
    let mut function_statement_ids: Vec<usize> = sierra_program
        .funcs
        .iter()
    ...
    for (idx, statement) in sierra_program.statements.iter().enumerate() {
    ...
    current_function.finalize(sierra_program.statements.len())?;
...
fn functions_statement_ids_to_offsets(
    ...
    segment_starts_statements
        .iter()
        .map(|start| statement_to_offset(*start))
        .collect()
}
...
fn get_segment_lengths(segment_starts_offsets: &[usize], bytecode_len: usize) -> Vec<usize> {
    ...
    let last_offset = segment_starts_offsets
        .last()
        .expect("Segmentation error: No function found.");
